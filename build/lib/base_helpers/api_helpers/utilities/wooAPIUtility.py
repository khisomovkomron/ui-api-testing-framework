from base_helpers.api_helpers.configs.hosts_config import WOO_API_HOSTS
from base_helpers.api_helpers.utilities.credentialsUtility import CredentialsUtility
import os
...
class WooAPIUtility(object):
    ...
    def assert_status_code(self):
        assert self.status_code == self.expected_status_code, f"Bad status code. Expected {self.expected_status_code}," \
                    f"Actual status code: {self.status_code}, url: {self.endpoint}, Response JSON: {self.rs_json}"
    ...
    def put(self, wc_endpoint, params=None, expected_status_code=201):
