...
class RequestsUtility(object):
    ...
    def post(self, endpoint, payload=None, headers=None, expected_status_code=201):
        ...
        try:
            rs_api = requests.post(url=self.url, data=json.dumps(payload), headers=headers, auth=self.auth, timeout=10)
    ...
    def get(self, endpoint, payload=None, headers=None, expected_status_code=200):
        ...
        self.url = self.base_url + endpoint
        rs_api = requests.get(url=self.url, data=json.dumps(payload), headers=headers, auth=self.auth, timeout=self.timeout)
        self.status_code = rs_api.status_code
        self.expected_status_code = expected_status_code
        self.rs_json = rs_api.json()
        self.assert_status_code()
